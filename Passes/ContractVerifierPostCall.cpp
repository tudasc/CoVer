...
void ContractVerifierPostCallPass::appendDebugStr(std::string Target, bool isTag, const CallBase* Provider, const std::set<const CallBase *> candidates, std::vector<ErrorMessage>& err, const Instruction* retLoc) {
    ...
    err.push_back({
        ...
        .text = "Did not find postcall function " + Target + (isTag ? " (Tag)" : "") + " with required parameters after "
                    + demangle(Provider->getCalledOperand()->getName()) + " at " + ContractPassUtility::getInstrLocStr(Provider) + " before exit point at " + ContractPassUtility::getInstrLocStr(retLoc),
