// RUN: %binaries/clangContracts --predefined-contracts --instrument-contracts %s -o %t.exe > %t.test_out 2>&1 && (COVER_COVERAGE_FOLDER="%t_coverage" %t.exe >> %t.test_out 2>&1 || true) && FileCheck %s < %t.test_out
